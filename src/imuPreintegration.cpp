...
public:
    ...
    */
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        Eigen::Affine3f imuOdomAffineLast = lidarOdomAffine * imuOdomAffineIncre;  // 在激光里程计基础上叠加IMU增量里程计
        float x, y, z, roll, pitch, yaw;
        pcl::getTranslationAndEulerAngles(imuOdomAffineLast, x, y, z, roll, pitch, yaw);
        
        // 发布当前imu时刻的imu里程计位姿(高频的，在激光里程计的基础之上还叠加了经过图优化的IMU数据)
        nav_msgs::Odometry laserOdometry = imuOdomQueue.back();// 这里做这一步实际上是为了取他的时间戳和frameid,具体的变换阵由下面的给出
        laserOdometry.pose.pose.position.x = x;
        laserOdometry.pose.pose.position.y = y;
        laserOdometry.pose.pose.position.z = z;
        laserOdometry.pose.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(roll, pitch, yaw);
        pubImuOdometry.publish(laserOdometry); // 发布高频里程计（与imu频率相同）这里已经不是增量里程计，而是真正的IMU里程计
        ...
        if (imuTime - last_path_time > 0.1)
        {
            ...
            if (pubImuPath.getNumSubscribers() != 0)
            {
                ...
                pubImuPath.publish(imuPath);  // 这里发布的是IMU里程计的路径
            }
        }
    }
};
...
public:
    ...
    // ISAM2优化器
    gtsam::ISAM2 optimizer;  // 图优化器
    gtsam::NonlinearFactorGraph graphFactors; // 因子结点
    gtsam::Values graphValues;  // 变量结点

    const double delta_t = 0;  // 激光里程计时间戳和IMU时间戳相差的时间
    ...
    */
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        if (systemInitialized == false)
        {
            ...
            // 添加里程计位姿先验因子
            prevPose_ = lidarPose.compose(lidar2Imu);  // 转换到IMU坐标系
        ...
        while (!imuQueOpt.empty())
        {
            ...
            {
                // 两帧imu数据时间间隔
                double dt = (lastImuT_opt < 0) ? (1.0 / 500.0) : (imuTime - lastImuT_opt);  // 官方默认使用的IMU是500Hz的
        ...
        gtsam::NavState propState_ = imuIntegratorOpt_->predict(prevState_, prevBias_);  // 优化的初始值
        // 变量节点赋初值
    ...
    */
    bool failureDetection(const gtsam::Vector3& velCur, const gtsam::imuBias::ConstantBias& biasCur)
    {
        ...
        if (vel.norm() > 30)  // norm()返回的是它与自己点乘开方后的值
        {
    ...
    */
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        // 用经过图优化之后的上一帧激光里程计时刻对应的状态、偏置，施加从该时刻开始到当前时刻的imu预计分量，得到当前时刻的状态
