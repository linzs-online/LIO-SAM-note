...
public:
    ...
    */
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            // 通过IMU里程计来的信息对当前帧位姿进行初始化，获得一个粗糙的T_wr
            updateInitialGuess();
            ...
            // 这个函数在回环触发的时候才会起作用。更新因子图中所有变量节点的位姿，也就是所有历史关键帧的位姿，更新里程计轨迹。
            correctPoses();
    ...
    */
    ...
    /**
     * Eigen格式的位姿变换，把数组转换成Eigen中的变换矩阵
    */
    ...
    */
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        int loopKeyPre;
        // not-used
        if (detectLoopClosureExternal(&loopKeyCur, &loopKeyPre) == false)  // 这个函数必返回false，直接看下面这个函数即可
        ...
        // ICP参数设置
        static pcl::IterativeClosestPoint<PointType, PointType> icp;  // 从模板类中实例化一个icp类

        icp.setMaxCorrespondenceDistance(historyKeyframeSearchRadius*2);  // 设置最大匹配距离
        icp.setMaximumIterations(100);  // 设置最大迭代次数

        icp.setTransformationEpsilon(1e-6);  // 设置收敛阈值
        icp.setEuclideanFitnessEpsilon(1e-6);

        icp.setRANSACIterations(0);  // 设置RANSAC算法迭代次数，剔除外点
    ...
    */
    void cornerOptimization()
    {
        ...
        // 遍历当前帧角点集合
        // 这里通过 #pragma 告诉编译器用多线程对for循环处理点云进行加速
        #pragma omp parallel for num_threads(numberOfCores)
    ...
    */
     ...
     * 公式推导：这里使用的是旋转矩阵对欧拉角进行求导，使用的是ZXY外旋的方式。详细推导参考我的博客：https://blog.csdn.net/weixin_40599145/article/details/127398733
    */
    bool LMOptimization(int iterCount)
    {
        ...
        for (int i = 0; i < laserCloudSelNum; i++) {
            ...
            // 点到直线距离、平面距离，作为观测值
            matB.at<float>(i, 0) = -coeff.intensity;  // 点到平面的距离
        }
        ...
        // 首次迭代，检查近似Hessian矩阵（J^T·J）是否退化，或者称为奇异，主要检查矩阵的奇异值是否过小
        if (iterCount == 0) {
            ...
            for (int i = 5; i >= 0; i--) {
                if (matE.at<float>(0, i) < eignThre[i]) {  // 对于奇异值过小的特征向量直接赋0
                    for (int j = 0; j < 6; j++) {
            ...
            matP = matV.inv() * matV2;  // P矩阵中
        }
        if (isDegenerate)
        {
            cv::Mat matX2(6, 1, CV_32F, cv::Scalar::all(0));
            matX.copyTo(matX2);
            matX = matP * matX2;   // 实际上就是对于奇异值过小的对应的解就直接赋0，其他按原增量方程解出来是什么就是什么
        }
        ...
        // cv::Mat matX_t;
        // cv::transpose(matX, matX_t);
        // ROS_INFO_STREAM(format(matX_t,cv::Formatter::FMT_C));
        float deltaR = sqrt(
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            for (int iterCount = 0; iterCount < 30; iterCount++)
            {
                ...
                // scan-to-map优化
                // 对匹配特征点计算Jacobian矩阵，观测值为特征点到直线、平面的距离，构建高斯牛顿方程，迭代优化当前位姿，存入transformTobeMapped
    ...
    */
    void transformUpdate()
    {
        ...
        transformTobeMapped[0] = constraintTransformation(transformTobeMapped[0], rotation_tollerance);  // 对数值做一定的限制
    ...
    */
    void addOdomFactor()
    {
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            gtsam::Pose3 poseFrom = pclPointTogtsamPose3(cloudKeyPoses6D->points.back());  // cloudKeyPoses6D这个数据结构是存储经因子图优化之后的位姿的，所以在此时它最后一个还是上一帧的位姿
