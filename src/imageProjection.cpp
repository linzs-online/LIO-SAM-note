...
private:
    ...
    pcl::PointCloud<PointXYZIRT>::Ptr laserCloudIn_temp;
    ...
    bool odomDeskewFlag;
    // 当前激光帧起止时刻对应imu里程计，该变换对应的平移增量；用于插值计算当前激光帧起止时间范围内，每一时刻的位置
    float odomIncreX;
    float odomIncreY;
    float odomIncreZ;
...
public:
    ...
    */
    ...
    {
        std::lock_guard<std::mutex> lock2(odoLock);  //releasing ownership in the destructor.
    ...
    */
    bool cachePointCloud(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        if (sensor == SensorType::VELODYNE)
        {
            ...
            // pcl::removeNaNFromPointCloud(*laserCloudIn_temp, *laserCloudIn, mapping); 
        }
    ...
    */
    bool deskewInfo()
    {
        ...
        // 要求imu序列的能够覆盖整个整个激光扫描帧，以便后面进行点云矫正
    ...
    */
    PointType deskewPoint(PointType *point, double relTime)
    {
        ...
        // 当前时刻激光点与第一个激光点的位姿变换
        Eigen::Affine3f transFinal = pcl::getTransformation(posXCur, posYCur, posZCur, rotXCur, rotYCur, rotZCur);  // 获得仿射变换矩阵
        Eigen::Affine3f transBt = transStartInverse * transFinal;

        // 对激光点进行畸变矫正，变换到当前激光束第一个激光点坐标系下
        PointType newPoint;
...
int main(int argc, char** argv)
{
    ...
    ros::MultiThreadedSpinner spinner(3); // 多线程处理回调任务
    spinner.spin();
